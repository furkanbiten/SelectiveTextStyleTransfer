...
def COCOText_inputs(batch_size, image_size):
  ...
  """
  # dataset = custom_dataset.CustomDataset('/home/raulgomez/datasets/COCO-Text/')
  ...
  with tf.name_scope('batch_processing'):
    ...
    for i in range(batch_size):
      ...
      img_and_masks.append([image, mask])

    images, masks = tf.train.batch_join(img_and_masks,batch_size=batch_size,capacity=2 * batch_size)

    images = tf.reshape(images, shape=[batch_size, image_size, image_size, 3])
    masks = tf.reshape(masks, shape=[batch_size, image_size, image_size, 3])
